...
def train(network, loss, x_train, y_train, x_val, y_val, epochs, batch_size, initial_learning_rate, verbose):
    ...
    training_losses, training_accuracies, validation_losses, validation_accuracies, validation_macro_f1_scores = [], [], [], [], []
    ...
    for epoch in range(epochs):
        ...
        val_output = x_val
        for layer in network:
            val_output = layer.forward_propagation(val_output) if not isinstance(layer, Dropout) else layer.forward_propagation(val_output, train=False)
        ...
        validation_macro_f1_scores.append(calculate_macro_f1_score(val_output, y_val))

        if verbose:
            print(f"Epoch: {epoch+1}, Train loss: {train_loss:.5f}, Train accuracy: {train_accuracy:.3f}, Val loss: {val_loss:.3f}, Val accuracy: {val_accuracy:.3f}, Val macro f1 score: {validation_macro_f1_scores[-1]:.3f}")
